<script lang="ts">
	...
	const doSSOLogin = async (token: string) => {
		try {
			...
			if (code !== 0) {
				messageBoxStore.push('提示訊息', message, [
					{
						text: '確認',
						callback: () => {
							goto('/404');
			...
			if (isAxiosError<SSOLoginResponse>(e)) {
				...
				if (typeof ssoLoginResponse === 'undefined') {
					messageBoxStore.push('提示訊息', '發生未知錯誤', [
						{
							text: '確認',
							callback: () => {
								goto('/404');
...
<div class="w-screen h-screen flex flex-row items-center justify-center bg-zinc-200">
	<div class="flex flex-col items-center justify-center bg-stone-800 rounded-xl px-6 py-6">
		...
		<div class="mt-4">
			<p class="text-zinc-200 text-md font-noto">打工人準備中</p>
		</div>
	</div>
	<div
		class="fixed bottom-5 left-[50%] translate-x-[-50%] flex flex-col items-center justify-center"
	>
		<p class="md:text-md text-sm text-stone-800 font-noto">Copyright © 2024 Workerrr</p>
		<p class="md:text-md text-sm text-stone-800 font-noto">All rights reserved</p>
	</div>
</div>
