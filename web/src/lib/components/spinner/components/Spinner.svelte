<script lang="ts">
	...
	import { createSpinnerStore } from '../store/';

	const state = createSpinnerStore();
</script>

<Dialog.Root open={$state.spinners.length > 0}>
	<Dialog.Portal>
		<Dialog.Overlay
			transition={fade}
		...
		>
			<div class="flex flex-col items-center justify-center bg-stone-800 rounded-xl px-6 py-6">
				...
				<div class="mt-4">
					<p class="text-zinc-200 text-md font-noto">
                        {#if typeof $state.spinners[0].message !== 'undefined'}
                            {$state.spinners[0].message}
						{:else}
							打工人處理中
						{/if}
                    </p>
